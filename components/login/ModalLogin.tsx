...
const ModalLogin:React.FC<Props> = ({openLogin,setOpenLogin,setOpenRegistro,setOpenRecuperarPassword}) =>{
    ...
    return(
        ...
        <div className={` justify-end ${ openLogin ? "flex" : "hidden"} `}>
            <div className="bg-white px-5 md:px-14  pb-10 absolute z-30 border border-slate-200 rounded-bl-2xl w-5/6 sm:w-4/6  md:w-3/5 lg:w-3/6 xl:w-1/3">
                ...
                <form action="" className="mt-10 pr-14 pl-6 ">
                    ...
                    <input type="email" value={email}  className="block my-3 w-full border border-slate-500 rounded-xl py-2 px-4" placeholder="example@gmail.com" onChange={(e)=>setEmail(e.target.value)}/>
                    <label className="mt-10 mb-5  text-xl" htmlFor="">Password</label>
                    <input type="password" value={password} className="block mt-3 w-full border border-slate-500 rounded-xl py-2 px-4" placeholder="*************" onChange={(e)=>setPassword(e.target.value)}/>
                    
                </form>
                <div className="pr-14 pl-6">
                        <button className="bg-black text-white w-full mt-10 py-2 rounded-xl" onClick={()=>loguearse()}>Iniciar sesión</button>
                    </div>
