...
export const Navbar = ()=> {
    ...
    const cantidadProductos = useAppSelector(state=> state.productos.cantidadProductos)
     ...
     const outHover = (nombre:string) =>{
        if(hoverSubProductos === false ){
          setHoverSubProductos(false)
        }
        if(nombre === 'SOLUCIONES' && hoverSubSoluciones === false){
          setHoverSubSoluciones(false)
        }
     }
    ...
    return(
        <>
          <nav className="md:pt-5 lg:pt-0 navbar hidden md:block bg-[#1E1E1E] lg:flex items-center justify-around relative w-full">
            ...
            <div className="flex items-center justify-center ">
              <ul className="flex">
                ...
                {navLinks.map(links=>(
                    <li className={`flex md:px-2 lg:px-4 items-center ${links.hover && 'subnavegacion'}`} key={links.nombre} onMouseEnter={()=>onHover(links.nombre)} onMouseLeave={()=>outHover(links.nombre)}>
                      <a href={links.url} className="text-[#D9D9D9] py-5">{links.nombre}</a>
                      {links.logo !== '' && links.logo } 
                    </li>
                ))}
              </ul>
              
              <div className=" hidden lg:block lg:ml-5 xl:ml-10 relative">
                {cantidadProductos === 0 ? '' : <div  className="bg-red-600 text-xs px-1 rounded-full text-white absolute top-2 left-4">{cantidadProductos}</div>}
